...
class StudiedTimeGroupByClassService {
  ...
  load = action(() => {
    this.fetch.fetch({
      url: `/reports/schools/${this.schoolId}/classes/${this.classId}/studied-time-groups`,
    }).then(() => {
      if (this.fetch.data) {
...
const studiedTimeGroupByClassService = new StudiedTimeGroupByClassService();

export default studiedTimeGroupByClassService;
